                            ...
                            <div class="collapse navbar-collapse">
                                <form class="navbar-form form-inline" action="" method="get">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <input type="text" name="p_Course_name" value='<?php echo ($_GET['p_Course_name']); ?>' class="form-control" placeholder="分类" />
                                        </div>
                                    </div>
                                    <div class="form-group">
<!--                                         <label class="control-label" for="input-mobile">所属小区：</label>
                                    ...
                                    <div class="form-group pull-right"> 
                                        ...
                                        <a href="<?php echo U('Admin/CourseType/addCourseType');?>" class="btn btn-primary">添加分类</a>
                                        <a href="javascript:void(0)" class="btn btn-danger pull-right del_cus del_all" style="margin-left: 2px;" data-url="<?php echo U('Admin/CourseType/ajaxDelCourseType');?>"><i class="fa fa-trash-o"></i> 删除</a>
                                    </div>
                                </form>
                            </div>
                        </nav>
                        <div class="table-responsive">
                            <table  class="table table-bordered table-striped">
                                ...
                                <tbody>
                                <?php if(empty($CourseType_list)): ?><tr role="row" align="center"><td colspan="6">暂无数据</td></tr>
                                    <?php else: ?>
                                    <?php if(is_array($CourseType_list)): $i = 0; $__LIST__ = $CourseType_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$customer): $mod = ($i % 2 );++$i;?><tr role="row">
                                            <td class="text-center"><input type="checkbox" class='fck' value="<?php echo ($customer["course_id"]); ?>" name="customer_id[]" /></td>
                                            <td><!-- <span data-url="<?php echo U('Admin/CourseType/gettree');?>" data-id="<?php echo ($customer["cate_Id"]); ?>" class="gettree" style="display:inline-block;width:15px;line-height:15px;border:1px solid #ddd;text-align:center">+</span> --><?php echo ($customer["cate_Name"]); ?></td>
                                           <!--  <td><?php echo ($customer["residential_quarters_name"]); ?></td> -->
                                            <td><?php echo ($customer["cate_Id"]); ?></td>
                                          <!--   <td><?php echo ($customer["person_range"]); ?></td> -->
                                            <td>
                                                <a class="btn btn-primary btn-sm" href="<?php echo U('Admin/CourseType/detailInfo',array('course_id'=>$customer['cate_Id']));?>">子类</a>
                                                <a class="btn btn-primary btn-sm" href="<?php echo U('Admin/CourseType/editCourseType',array('course_id'=>$customer['cate_Id']));?>">编辑</a>
                                                <a class="btn btn-danger btn-sm del_cus" href="javascript:void(0)" data-url="<?php echo U('Admin/CourseType/ajaxDelCourseType');?>" data-id="<?php echo ($customer["cate_Id"]); ?>">删除</a>
                                            </td>
